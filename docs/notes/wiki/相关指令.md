...
- `/ysm model set <player> <model_id> <texture_id> [ignore_auth]` 将特定模型、材质赋予给某个玩家，`texture_id` 可填入 `-` 以使用默认的材质。最后的 `ignore_auth` 参数是可选参数，当设置为 true 时，会无视模型授权，强制为玩家赋予模型；
...
- `/ysm model disable <player> [true | false]` 2.5.2 新增指令，用于禁用某个玩家的模型，设为 true 时对应的玩家将不再显示 YSM 模型，回退为原版模型。
...
- `/ysmclient molang watch add [pre|post] <name> <expr>` 向自定义调试屏幕添加 molang 表达式，会实时计算和更新；
    ...
    - 2.5.2 版本后可对准使用 YSM 模型的实体按 `Alt` + `B` 对目标实体进行调试
- `/ysmclient molang watch remove <name>` 移除先前添加的某个 molang；
- `/ysmclient molang watch clear` 清除先前添加的所有 molang。
- `/ysmclient debug <selector>` 2.5.2 新增指令，可以精确匹配对应的实体进行调试。
- `/ysmclient molang execute <expr>` 2.5.2 新增指令，可在当前调试实体上执行 Molang 表达式。
